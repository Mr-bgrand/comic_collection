...
export async function importCardImages(manifestFile,containerFile='data/cards/psa-vault.json') {
  ...
  for(const entry of manifest.records){
    ...
    if(!entry.front&&!entry.back){card.scanStatus=entry.status==='no-scans-on-cert-page'?'no-scans-on-cert-page':'not-fetched';card.scanCheckedAt=manifest.discoveredAt;missing++;continue;}
    try{
      for(const side of ['front','back']){
        const url=entry[side];if(!url)continue;
        const filename=entry.grader.replaceAll(' ','')+'_'+entry.cert+'_'+side.toUpperCase()+'.jpg',destination='data/images/'+filename;
