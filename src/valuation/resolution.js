...
import {historyCaution} from './evidence-strength.js';
...
export function resolveValuation(record) {
 ...
 if(o)return {value:o.value,currency:o.currency,basis:o.basis,source:o.source.name,url:o.source.url,asOf:o.source.asOf,retrievedAt:o.source.retrievedAt,observationId:o.id,locked:!!record.valuation.selection.locked,provisional:o.provisional===true,caution:historyCaution(o),legacy:false};
 if(record.valuation?.legacyInvalidatedAt)return null;
 if(isRaw(record)&&!assessedCondition(record))return null;
 const market=marketValuation(record);if(market)return {...market,basis:market.source==='psa-grade-comparison'?'psa-comparison':'guide',asOf:market.asOf||null,caution:historyCaution(market),legacy:true};
 if(typeof record.manual?.value==='number'&&Number.isFinite(record.manual.value)&&record.manual.value>=0)return {...record.manual,basis:'owner',source:record.manual.source||'Owner estimate',asOf:record.manual.asOf||null,caution:historyCaution({basis:'owner'}),legacy:true};
 return null;
}
