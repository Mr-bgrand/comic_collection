...
Example is a fictional fixture. Obtain current revision and exact identity from the local collection; independently verify source edition before declaring exact. Import does not turn a copied identity into proof. `asOf` is the provider's date or explicit null when undated. Retrieval time is separate. Values are decimal USD numbers, never strings. Sold observations require `saleCount`. One/two verified sales can be selected only with `provisional:true`, a nonempty `provisionalReason`, and explicit acceptance; automatic acceptance rejects provisional observations. PSA evidence for TAG uses `basis:"psa-comparison"`, `match.grader:"PSA"` and the same numeric grade. Guide comparison uses `evidenceKind:"guide"` without saleCount. Actual transaction-derived comparison uses `evidenceKind:"sold-comps"`.

Public and printed prices gain `*` for fewer than five matching sales, undocumented supporting history, owner estimates, or explicit provisional reasons. The marker never relaxes identity, grade, signer or raw-condition matching. Reviewed comic metadata may include `signatures:[{name,date?,witnessed?}]`; dates must be valid and nonfuture. Signer changes invalidate prior identity matches while preserving the audit history.
