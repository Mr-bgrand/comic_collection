...
import {ESTIMATE_NOTE} from './evidence-strength.js';
...
export function valuationAmount(record) { const v=resolveValuation(record);return v?valuationMoney(v.value)+(v.caution?'*':''):'Not yet valued'; }
...
export function publicValuation(record,options={}) {
 ...
 return {value:current?.value??null,valueText:valuationAmount(record),valueCaution:current?.caution??null,source:current?label(record,current):null,date:current?.asOf?.slice(0,10)||null,evidence:current?.url||null,basis:current?.basis??null,
...
export function valuationSourceText(record,{includeCaution=true}={}) {
 const current=resolveValuation(record);
 return current?[label(record,current),current.asOf?'Source as of '+current.asOf.slice(0,10):'Source undated',...(includeCaution&&current.caution?[ESTIMATE_NOTE+' '+current.caution]:[])].join(' · '):'';
}
