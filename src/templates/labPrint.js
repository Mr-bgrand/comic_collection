...
import { displayTitle, gradeLabel, effectiveValue, formatMoney, graderOf } from '../model.js';
import { marketValuation, marketValueLabel } from '../card-valuation.js';
...
export function renderCollectionMaster(collection) {
  ...
  const rows = entries.map(({ comic: c, group: b }, i) => {
    const physical = !!b.bin || b.physical === true && !b.virtual;
    ...
    const valuation = marketValuation(c);
    const basis = valuation ? [marketValueLabel(c), valuation.asOf || valuation.fetchedAt || 'date not supplied'].join(' · ') : c.manual?.value != null ? `Owner estimate · ${c.manual.setAt || 'date not supplied'}` : '';
