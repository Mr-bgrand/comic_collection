...
**05 Singularity / Event Horizon** is the selected cinematic direction, using ThreeUI's Hyperspace as a motion reference. A deep tunnel of real two-sided scans and accelerating ice-blue streaks surrounds the camera. The owner replaced the circular black hole and accretion rings with a continuous Milky Way backdrop: a diagonal band of fine stars, irregular dark dust lanes, cool outer haze and a soft pearl center. Keep its drift barely perceptible and the intact scans dominant. Give the intact scan five full seconds after its reveal. The entire active cycle is 11.2 seconds: approach 3.4, crossing .8, reveal 1.2, hold 5, departure .8. The scene uses existing atlases and bounded instanced batches rather than new renderer packages.

Scanned copies play in a stable mixed sequence with All / Comics / Cards scope. Pause, direct interaction, dialogs and hidden tabs settle the current copy for inspection and stop arrivals. Resume begins with its viewing hold; Replay flight restarts the current journey. Motion off gives a still presentation with manual navigation. Ambient hides and removes controls from keyboard focus, enlarges the art, and leaves an explicit Show controls action; Escape exits Ambient. Mobile portrait reserves space for the complete scan and lower controls; compact landscape places scope, metadata and playback beside the image. Link directly with `?view=singularity` or `?view=singularity&ambient=1`. This is a local ambient display, not an installed operating-system screensaver.
