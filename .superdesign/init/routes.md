...
- /: src/templates/indexPage.js (shared.js document wrapper; 3D builds its own document).
- /dashboard/: src/templates/dashboard.js (shared.js document wrapper; 3D builds its own document).
- /wall/: src/templates/wallPage.js (shared.js document wrapper; 3D builds its own document).
- /wall/3d/: src/templates/wall3dPage.js (shared.js document wrapper; 3D builds its own document).
- /bin/:bin/: src/templates/binPage.js (shared.js document wrapper; 3D builds its own document).
- /bin/:bin/label.html: src/templates/label.js (shared.js document wrapper; 3D builds its own document).
- /bin/:bin/sheet.html: src/templates/sheet.js (shared.js document wrapper; 3D builds its own document).
- local editor :4173: src/templates/editorPage.js (shared.js document wrapper; 3D builds its own document).

Home: searchable bin index. Dashboard: monetary summary, charts, sortable table. Wall: cover gallery. Vault: WebGL wall/longbox/helix/orbit. Bin: complete records and scans. Local editor: owner tools. Remote editor is separately deployed from editor/public/index.html. PSA/TAG routes exist only in the draft spec.
